...
#define LOAD_INT8_OPERAND() \
    ...
    case getstatic:
      {
        ...
        const_utf8* mydesc = (const_utf8*)constant_pool[nt->desc];
  ...
  */
  ...
  for (i = code_length; i >= 0; i--) {
    local_context* mynext;
    for (ctx = context[i]; ctx != NULL; ctx = mynext) {
      mynext = ctx->next;
      delete ctx;
    }
  }
...
field_desc* method_desc::getNew() {
  char* myname = (char *)malloc((size_t)10);
  strcpy(myname, "<new>");
  //  assert(equal_descs.size() < 10000); // 4 characters allowed
  assert(new_cnt < 10000); // 4 characters allowed
  snprintf(myname + strlen(myname), 4, "%d", new_cnt++);
  const char* fd_name = stringPool.add(myname);
  free(myname);
